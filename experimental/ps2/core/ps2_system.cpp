...
namespace ps2 {
Ps2System::Ps2System():cdvd_(iop_intc_),iop_bus_(iop_ram_,iop_hw_,hw_,iop_intc_,cdvd_,bios_),bus_(ram_,scratchpad_,hw_,iop_hw_,iop_ram_,gs_,gs_core_,bios_),vu0_(bus_,gs_core_,0x11000000u,0x11004000u,0x0FFFu,0x100038D0u,0x100038E0u,false),vu1_(bus_,gs_core_),ee_(bus_,&vu0_),iop_(iop_bus_){gs_core_.attach_privileged(gs_);vif0_dma_.attach_vu0(vu0_);vif1_dma_.attach_vu1(vu1_);reset();}
void Ps2System::reset(u32 entry_point){ram_.reset();scratchpad_.reset();bus_.reset();hw_.reset();iop_hw_.reset();iop_intc_.reset();cdvd_.reset();iop_ram_.reset();iop_bus_.reset();gs_.reset();gs_core_.reset();gs_display_.reset();scheduler_.reset();video_timing_.reset();gif_dma_.reset();vif0_dma_.reset();vif1_dma_.reset();sif_dma_.reset();spr_dma_.reset();vu0_.reset();vu1_.reset();ee_.reset(entry_point);iop_.reset(Bios::kResetVector);bios_started_=false;reset_instruction_=0;iop_reset_instruction_=0;ee_iop_phase_=0;}
...
bool Ps2System::step_ee(std::string& error){error.clear();if(!bios_started_){error="BIOS has not been started.";return false;}if(halted()){error=halt_reason();return false;}if(!ee_.step(error)){error="EE halted: "+error;return false;}if(!gif_dma_.service(bus_,gs_core_,error)){error="GIF DMA: "+error;return false;}if(!vu0_.running())ee_.sync_vu0_to_micro();if(!vif0_dma_.service(bus_,error)){error="VIF0 DMA: "+error;return false;}if(!vif1_dma_.service(bus_,gs_core_,gs_,error)){error="VIF1 DMA: "+error;return false;}if(!sif_dma_.service(bus_,iop_bus_,iop_intc_,error)){error="SIF DMA: "+error;return false;}if(!spr_dma_.service(bus_,error)){error="SPR DMA: "+error;return false;}if(vu0_.running()){ee_.set_vu0_micro_running(true);std::string vu_error;vu0_.run(256,vu_error);if(!vu_error.empty()){error="VU0: "+vu_error;return false;}if(!vu0_.running()){ee_.set_vu0_micro_running(false);ee_.sync_vu0_from_micro();}}if(vu1_.running()){std::string vu_error;vu1_.run(256,vu_error);if(!vu_error.empty()){error="VU1: "+vu_error;return false;}}scheduler_.run_until(scheduler_.now()+1,{});const u64 fields_before=video_timing_.fields_started();video_timing_.tick(1,hw_,iop_intc_);if(video_timing_.fields_started()!=fields_before){gs_.raise_vsync();gs_display_.update(gs_,gs_core_.vram());}if(gs_.irq_pending())hw_.raise_intc(0);return advance_iop_for_ee_step(error);}
