...
public:
  ...
  void *copy_code(const void *src, size_t size) {
    ...
    // x86/x64 has coherent instruction and data caches. Generated bytes are
    // visible to instruction fetch without an OS-level cache-flush syscall.
    // Avoid paying FlushInstructionCache() for every tiny translated block.
    (void)dst;
    (void)size;
#elif defined(_WIN32)
